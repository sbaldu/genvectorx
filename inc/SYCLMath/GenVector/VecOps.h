...
template <class Scalar, class LVector>
__global__ void InvariantMassesKernel(LVector *v1, LVector *v2, Scalar *m,
                                      size_t N) {
  int id = blockDim.x * blockIdx.x + threadIdx.x;
  if (id < N) {
...
Scalar *InvariantMasses(LVector *v1, LVector *v2, const size_t N,
...

  {
    ...
    InvariantMassesKernel<<<N / local_size + 1, local_size>>>(d_v1, d_v2,
                                                              d_invMasses, N);

    ERRCHECK(cudaMemcpy(invMasses, d_invMasses, N * sizeof(Scalar),
...
Scalar *InvariantMass(LVector *v1, const size_t N, const size_t local_size) {
...
#endif
  {
    ...
    InvariantMassKernel<<<N / local_size + 1, local_size>>>(d_v1, d_invMasses,
                                                            N);
...
template <class AccScalar, class AccLVector> class InvariantMassKernel {
public:
...
template <class AccScalar, class AccLVector> class InvariantMassesKernel {
public:
